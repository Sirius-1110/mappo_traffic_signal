...
class SampleSerialCollector(ISerialCollector):
        ...
        while collected_sample < n_sample:
            ...
            for env_id, timestep in timesteps.items():
                ...
                if timestep.done:
                    ...
                    # (roy): Fix version conflict 
                    reward = timestep.info['final_eval_reward']
                    queue_len = timestep.info['queue_len']
                    wait_time = timestep.info['wait_time']
                    delay_time = timestep.info['delay_time']
                    pressure = timestep.info['pressure']
                    info = {
                        'reward': reward,
                        'queue_len': queue_len,
                        'wait_time': wait_time,
    ...
    def _output_log(self, train_iter: int) -> None:
        ...
        if (train_iter - self._last_train_iter) >= self._collect_print_freq and len(self._episode_info) > 0:
            ...
            episode_queue_len = [d['queue_len'] for d in self._episode_info]
            episode_wait_time = [d['wait_time'] for d in self._episode_info]
            episode_delay_time = [d['delay_time'] for d in self._episode_info]
            episode_pressure = [d['pressure'] for d in self._episode_info]
            ...
            info = {
                ...
                'queue_len': np.mean(episode_queue_len),
                'wait_time': np.mean(episode_wait_time),
                'delay_time': np.mean(episode_delay_time),
                'pressure': np.mean(episode_pressure),
